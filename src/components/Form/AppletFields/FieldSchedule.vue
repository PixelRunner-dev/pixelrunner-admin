...
<template>
  <DFieldset :legend="$t('fieldScheduleComponent.legend')" class="component--field-schedule my-4 gap-4">
    ...
    <template v-if="schedule">
      <DFormControl
        class="gap-1 min-w-0 max-w-full overflow-x-auto overflow-y-hidden min-w-0 scrollbar-thin"
        tabindex="-1"
      >
        <div class="grid grid-cols-7 gap-2 p-1 w-max">
          <DLabel
            ...
            @keyup.enter.space="
              scheduleDays[daysOfWeek.indexOf(d)] = !scheduleDays[daysOfWeek.indexOf(d)]
            "
          >
